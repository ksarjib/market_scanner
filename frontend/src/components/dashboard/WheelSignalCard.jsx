...
// 1. Import the shared dropdown menu
import StockActionsMenu from './StockActionsMenu';

const WheelSignalCard = ({ 
  item, 
  onLogTrade,
  // 2. Accept all the action props needed for the menu
  onTogglePortfolio,
  onToggleWatchlist,
  onToggleIgnore,
  onOpenChart,
  ...
  const colorClass = isPut ? 'emerald' : 'indigo'; 

  return (
    <motion.div 
        onClick={() => onOpenChart && onOpenChart(item.symbol)}
        initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} 
        // 3. IMPORTANT: 'hover:z-50' lets the dropdown overlap other cards
        className={`relative rounded-xl p-4 border bg-gradient-to-b from-[#0f172a] to-[#020617] shadow-lg group hover:ring-1 hover:ring-${colorClass}-500/50 transition-all border-slate-800 hover:z-50 cursor-pointer`}
    >
      {/* Header */}
      <div className="flex justify-between items-start mb-3">
        ...
        {/* TOP RIGHT: Badge + The New "+" Dropdown */}
