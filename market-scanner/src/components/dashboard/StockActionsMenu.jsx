      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            {/* 1. Portfolio */}
            <button 
              onClick={() => { onTogglePortfolio(item.symbol, item.is_in_portfolio); setIsOpen(false); }}
              className="w-full text-left px-4 py-3 hover:bg-slate-800 flex items-center justify-between group border-b border-slate-800/50 transition-colors"
            >
            ...
            {/* 2. Watchlist */}
            <button 
              onClick={() => { onToggleWatchlist(item.symbol, item.is_watched); setIsOpen(false); }}
              className="w-full text-left px-4 py-3 hover:bg-slate-800 flex items-center justify-between group border-b border-slate-800/50 transition-colors"
            >
              <div className="flex items-center gap-3">
            ...
            {/* 3. View Chart */}
            <button 
              onClick={() => { if(onOpenChart) onOpenChart(item.symbol); setIsOpen(false); }}
              className="w-full text-left px-4 py-3 hover:bg-slate-800 flex items-center gap-3 group border-b border-slate-800/50 transition-colors"
            >
            ...
            {/* 4. Latest News */}
            <button 
              onClick={() => { if(onOpenNews) onOpenNews(item.symbol); setIsOpen(false); }}
              className="w-full text-left px-4 py-3 hover:bg-slate-800 flex items-center gap-3 group border-b border-slate-800/50 transition-colors"
            >
            ...
            {/* 5. Ignore */}
            <button 
                onClick={() => { onToggleIgnore(item.symbol); setIsOpen(false); }}
                className="w-full text-left px-4 py-3 hover:bg-rose-900/20 flex items-center gap-3 group transition-colors"
              >
